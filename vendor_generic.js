...
export function renderGeneric(ctx, maker, path=[], overseas=false){
  ...
  if(path.length===0){
    ...
    const row=rowBox();
    row.append(chip("はい",()=>{ pushCrumb("保証：はい"); nav("generic", [ctx, maker, ["w_yes"], overseas]); }));
    row.append(chip("いいえ",()=>{ pushCrumb("保証：いいえ"); nav("generic", [ctx, maker, ["w_no"], overseas]); }));
    panel.append(row);
    return;
  }
  if(path[0]==="w_yes"){
    if(!overseas){
      ...
      if(path.length===1){
        ...
        const row=rowBox();
        row.append(chip("はい",()=> nav("generic", [ctx, maker, ["w_yes","n_yes"], true])));
        row.append(chip("いいえ",()=> nav("generic", [ctx, maker, ["w_yes","n_no"], true])));
        panel.append(row);
        return;
      }
      if(path[1]==="n_yes"){
