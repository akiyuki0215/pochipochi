...
export function renderVAIO(ctx, step="chooseMaker"){
  ...
  if(step==="chooseMaker"){
    ...
    const row=rowBox();
    row.append(chip("Sony株式会社のVAIO", ()=>{ nav("vaio", [ctx, "sony"]); }));
    row.append(chip("VAIO株式会社のVAIO", ()=>{ nav("vaio", [ctx, "vaio_corp_q"]); }));
    panel.append(row);
    return;
  }
  if(step==="sony"){
  ...
  if(step==="vaio_corp_q"){
    ...
    const row=rowBox();
    row.append(chip("はい", ()=>{ nav("vaio", [ctx, "vaio_corp_yes"]); }));
    row.append(chip("いいえ", ()=>{ nav("vaio", [ctx, "vaio_corp_no"]); }));
    panel.append(row);
    return;
  }
